...
public class WhiteboardService {
    ...
    public Mono<WhiteboardOpenResponse> openWhiteboardInSession(WhiteboardOpenRequest request) {
        ...
        // 🔐 Force studentId from VideoSession for SOLO/DEMO/RECURRING
        if (request.getSessionType() == null ||
                request.getSessionType() == SessionType.SOLO ||
                request.getSessionType() == SessionType.DEMO ||
                request.getSessionType() == SessionType.RECURRING) {

            String bookedStudentId = videoSession.getStudentId();
            log.info("🎯 Binding whiteboard studentId to VideoSession.studentId = {}", bookedStudentId);
            request.setStudentId(bookedStudentId);
            request.setEnrolledStudentIds(List.of(bookedStudentId));
        }
                ...
                .flatMap(roomData -> {
                    ...
                    return agoraClient.generateRoomToken(uuid, "admin")
                            .map(roomToken -> {
                                ...
                                log.info("🧱 Building WhiteboardRoom: classSessionId={}, teacherId={}, studentId={}, enrolledStudentIds={}",
                                ...
                                log.info("💾 Saved WhiteboardRoom: id={}, classSessionId={}, studentId={}, enrolledStudentIds={}",
                                        savedRoom.getId(),
                                        savedRoom.getClassSessionId(),
                                        savedRoom.getStudentId(),
                                        savedRoom.getEnrolledStudentIds());
